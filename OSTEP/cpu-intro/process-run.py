...
# To make Python2 and Python3 behave the same
...
class Scheduler:
    def __init__(self, process_switch_behaviour, io_done_behaviour, io_length):
        # Keep set of instructions for each of the processes
        self.proc_info = {}
        self.process_switch_behaviour = process_switch_behaviour
        self.io_done_behaviour = io_done_behaviour
    ...
    def run(self):
        ...
        # Track outstanding I/Os, per process
        ...
        while self.get_num_active() > 0:
            ...
            for pid in range(len(self.proc_info)):
                if clock_tick in self.io_finish_times[pid]:
                    ...
                    if self.io_done_behaviour == IO_RUN_IMMEDIATE:
                        # IO_RUN_IMMEDIATE
                        ...
                        # IO_RUN_LATER
                        if self.process_switch_behaviour == SCHED_SWITCH_ON_END and self.get_num_runnable() > 1:
                            # This means the process that issued the I/O shoudl be run
                            self.next_proc(pid)
            ...
            # If this is an I/O start instruction, switch to waiting state
            # and add an I/O completion in the future
            if instruction_to_execute == DO_IO:
                self.move_to_wait(STATE_RUNNING)
                self.io_finish_times[self.curr_proc].append(clock_tick + self.io_length + 1)

                if self.process_switch_behaviour == SCHED_SWITCH_ON_IO:
                    self.next_proc()
...
parser.add_option('-S', '--switch', default='SWITCH_ON_IO', help='when to switch between processes: SWITCH_ON_IO, SWITCH_ON_END', action='store', type='string', dest='process_switch_behaviour')
parser.add_option('-I', '--iodone', default='IO_RUN_LATER', help='type of behaviour when IO ends: IO_RUN_LATER, IO_RUN_IMMEDIATE', action='store', type='string', dest='io_done_behaviour')
...
# assert(options.process_switch_behaviour == SCHED_SWITCH_ON_IO or options.process_switch_behaviour == SCHED_SWITCH_ON_END)
# assert(options.io_done_behaviour == IO_RUN_IMMEDIATE or options.io_done_behaviour == IO_RUN_LATER)

s = Scheduler(options.process_switch_behaviour, options.io_done_behaviour, options.io_length)
...
if options.solve == False:
    ...
    print("Important behaviours:")
    
    print("  System will switch when ", end='')
    if options.process_switch_behaviour == SCHED_SWITCH_ON_IO:
    ...
    if options.io_done_behaviour == IO_RUN_IMMEDIATE:
